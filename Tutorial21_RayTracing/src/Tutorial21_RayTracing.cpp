...
void Tutorial21_RayTracing::CreateRayTracingPSO()
{
    ...
    // are only required for compatibility with OpenGL, and rayz
    ...
    RefCntAutoPtr<IShader> pCubePrimaryHit, pGroundHit, pGlassPrimaryHit, pSpherePrimaryHit, pSphereMetalHit, pSphereLambertianHit, pLambertianPrimaryHit, pMirrorPrimaryHit;
    {
        ...
        ShaderCI.Desc.Name     =    "Lambertian primary ray closest hit shader";
        ShaderCI.FilePath      =    "CubeLambertianHit.rchit";
        ShaderCI.EntryPoint    =    "main";
        m_pDevice->CreateShader(ShaderCI, &pLambertianPrimaryHit);
        VERIFY_EXPR(pLambertianPrimaryHit != nullptr);

                ShaderCI.Desc.Name  = "Mirror primary ray closest hit shader";
        ShaderCI.FilePath   = "CubeMirrorHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pMirrorPrimaryHit);
        VERIFY_EXPR(pMirrorPrimaryHit != nullptr);
        ...
        ShaderCI.Desc.Name       = "Sphere metal primary ray closest hit shader";
        ShaderCI.FilePath        = "SphereMetalHit.rchit";
        ShaderCI.EntryPoint      = "main";
        m_pDevice->CreateShader(ShaderCI, &pSphereMetalHit);
        VERIFY_EXPR(pSphereMetalHit != nullptr);

        ShaderCI.Desc.Name  = "Sphere lambertian primary ray closest hit shader";
        ShaderCI.FilePath   = "SphereLambertianHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pSphereLambertianHit);
        VERIFY_EXPR(pSphereLambertianHit != nullptr);
    ...
    PSOCreateInfo.AddTriangleHitShader("LambertianPrimaryHit", pLambertianPrimaryHit);

    PSOCreateInfo.AddTriangleHitShader("MirrorPrimaryHit", pMirrorPrimaryHit);
    ...
    PSOCreateInfo.AddProceduralHitShader("SphereMetal", pSphereIntersection, pSphereMetalHit);

    PSOCreateInfo.AddProceduralHitShader("SphereLambertian", pSphereIntersection, pSphereLambertianHit);
...
void Tutorial21_RayTracing::CreateProceduralBLAS()
{
    ...
    const HLSL::BoxAttribs Boxes[] = {
        HLSL::BoxAttribs{-2.5f, -2.5f, -2.5f, 2.5f, 2.5f, 2.5f},
        HLSL::BoxAttribs{-0.7f, -0.7f, -0.7f, 0.7f, 0.7f, 0.7f}};
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    static constexpr int MaxTotal  = BaseCount + MaxSmallSpheres + MaxSmallCubes;
    ...
    if (smallSpherePositions.empty())
    {
        ...
        std::uniform_real_distribution<float> distX(-20.0f, 20.0f), distZ(-20.0f, 20.0f);
        const float                           yFloor        = -5.0f;
        const float                           sphereRadius  = 1.0f;
        const float                           minSphereDist = sphereRadius * 2 + 0.1f;
        ...
        for (int i = 0; i < MaxSmallSpheres; ++i)
        {
            ...
            for (int tries = 0; tries < maxTries; ++tries)
            {
                ...
                valid = true;
                for (auto& p : smallSpherePositions)
                {
                    if (length(p - pos) < minSphereDist)
                    {
                        valid = false;
                        break;
                    }
            ...
            smallSpherePositions.push_back(pos);
        }
    }
    ...
    if (smallCubePositions.empty())
    {
        ...
        std::mt19937                          rng{std::random_device{}()};
        std::uniform_real_distribution<float> distX(-20.0f, 20.0f), distZ(-20.0f, 20.0f);
        const float                           yFloor            = -5.5f;
        const float                           cubeRadius        = 1.0f;
        const float                           sphereRadius      = 1.0f;
        const float                           minCubeCubeDist   = cubeRadius * 2 + 0.1f;
        const float                           minCubeSphereDist = cubeRadius + sphereRadius + 0.1f;
        const int                             maxTries          = 50;
        ...
        for (int i = 0; i < MaxSmallCubes; ++i)
        {
            ...
            for (int tries = 0; tries < maxTries; ++tries)
            {
                ...
                valid = true;
                for (auto& c : smallCubePositions)
                {
                    if (length(c - pos) < minCubeCubeDist)
                    {
                        valid = false;
                        break;
                    }
                ...
                for (auto& s : smallSpherePositions)
                {
                    if (length(s - pos) < minCubeSphereDist)
                    {
                        valid = false;
                        break;
                    }
            ...
            smallCubePositions.push_back(pos);
        }
    }
    ...
    if (!m_pTLAS)
    {
        ...
        TLASDesc.Name             = "TLAS";
        TLASDesc.MaxInstanceCount = MaxTotal;
    ...
    if (!m_InstanceBuffer)
    {
        ...
        BuffDesc.Usage     = USAGE_DEFAULT;
        BuffDesc.BindFlags = BIND_RAY_TRACING;
        BuffDesc.Size      = TLAS_INSTANCE_DATA_SIZE * MaxTotal;
    ...
    // Setup instances
    TLASBuildInstanceData Instances[MaxTotal] = {};
    ...
    const float angle45       = PI_F / 4;
    auto        rotY          = float3x3::RotationY(angle45);
    auto        scale         = float3x3::Scale(2.5f, 2.5f, 2.5f);
    float3x3    rotationScale = rotY * scale;


    Instances[1].InstanceName = "Cube Instance";
    Instances[1].pBLAS        = m_pCubeBLAS;
    Instances[1].Mask         = TRANSPARENT_GEOM_MASK;
    Instances[1].Transform.SetRotation(rotationScale.Data());
    Instances[1].Transform.SetTranslation(0.0f, -3.0f, 0.0f);


    Instances[2].InstanceName = "Cube Instance 2";
    Instances[2].pBLAS        = m_pCubeBLAS;
    Instances[2].Mask         = TRANSPARENT_GEOM_MASK;
    Instances[2].Transform.SetRotation(rotationScale.Data());
    Instances[2].Transform.SetTranslation(-8.0f, -3.0f, 0.0f);

    Instances[3].InstanceName = "Cube Instance 3";
    Instances[3].pBLAS        = m_pCubeBLAS;
    Instances[3].Mask         = TRANSPARENT_GEOM_MASK;
    Instances[3].Transform.SetRotation(rotationScale.Data());
    Instances[3].Transform.SetTranslation(8.0f, -3.0f, 0.0f);
        ...
        for (int i = 0; i < MaxSmallSpheres; ++i)
    {
        ...
        inst.Transform.SetTranslation(pos.x, -5.0f, pos.z);
    }
    
    static std::string smallCubeNames[MaxSmallCubes];

    for (int j = 0; j < MaxSmallCubes; ++j)
    {
        int   idx         = BaseCount + MaxSmallSpheres + j;
        auto& inst        = Instances[idx];

        smallCubeNames[j] = "GlassCube" + std::to_string(j);
        inst.InstanceName = smallCubeNames[j].c_str();
        inst.pBLAS        = m_pCubeBLAS;
        inst.Mask         = (j < m_NumScatterCubes ? TRANSPARENT_GEOM_MASK : 0u);

        auto        scale   = float3x3::Scale(0.5f, 0.5f, 0.5f);
        float3x3 rotationScale = rotY * scale;
        inst.Transform.SetRotation(rotationScale.Data());

        auto pos = smallCubePositions[j];
        inst.Transform.SetTranslation(pos.x, -5.5f, pos.z);
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Cube Instance", PRIMARY_RAY_INDEX, "GlassPrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Cube Instance 2", PRIMARY_RAY_INDEX, "LambertianPrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Cube Instance 3", PRIMARY_RAY_INDEX, "MirrorPrimaryHit");
    // clang-format on

    // Bind small spheres in a loop
    static constexpr int NumSmallSpheres = MaxSmallSpheres;
    const char*          MaterialsSphere[3]    = {"SphereMetal","SpherePrimaryHit","SphereLambertian"};
    for (int i = 0; i < NumSmallSpheres; ++i)
    {
        char instanceName[32];
        std::snprintf(instanceName, sizeof(instanceName), "SmallSphere%d", i);

        const char* hitGroupSphere = MaterialsSphere[i % 3];
    ...
    const char*          MaterialsCubes[2]  = {"GlassPrimaryHit", "LambertianPrimaryHit"};
   static constexpr int NumSmallCubes = MaxSmallCubes;
    for (int i = 0; i < MaxSmallCubes; ++i)
    {
        char instanceName[32];
        std::snprintf(instanceName, sizeof(instanceName), "GlassCube%d", i);

        const char* hitGroupCube = MaterialsCubes[i % 2];
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Initialize constants.
    {
        ...
        m_Constants.GlassMaterialColor       = {0.498f, 1.000f, 0.831f};
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Separator();
        ImGui::Text("Scattered Spheres");
        if (ImGui::SliderInt("Sphere Count", &m_NumScatterSpheres, 0, MaxSmallSpheres))
        {
            // rebuild sphere lists
            m_ScatterSpherePositions.clear();
            m_ScatterSphereNames.clear();
            m_ScatterSpherePositions.reserve(m_NumScatterSpheres);
            m_ScatterSphereNames.reserve(m_NumScatterSpheres);
            std::uniform_real_distribution<float> distX(-12.0f, 12.0f), distZ(-12.0f, 12.0f);
            const float                           yFloor = -3.0f;
            for (int i = 0; i < m_NumScatterSpheres; ++i)
            {
                m_ScatterSpherePositions.emplace_back(distX(m_Rng), yFloor, distZ(m_Rng));
                m_ScatterSphereNames.push_back("SmallSphere" + std::to_string(i));
            }
        }

        ImGui::Separator();
        ImGui::Text("Scattered Cubes");
        if (ImGui::SliderInt("Cube Count", &m_NumScatterCubes, 0, MaxSmallCubes))
        {
            // rebuild cube lists
            m_ScatterCubePositions.clear();
            m_ScatterCubeNames.clear();
            m_ScatterCubePositions.reserve(m_NumScatterCubes);
            m_ScatterCubeNames.reserve(m_NumScatterCubes);
            std::uniform_real_distribution<float> distX(-12.0f, 12.0f), distZ(-12.0f, 12.0f);
            const float                           yFloor = -3.0f;
            for (int i = 0; i < m_NumScatterCubes; ++i)
            {
                m_ScatterCubePositions.emplace_back(distX(m_Rng), yFloor, distZ(m_Rng));
                m_ScatterCubeNames.push_back("GlassCube" + std::to_string(i));
            }
        }
